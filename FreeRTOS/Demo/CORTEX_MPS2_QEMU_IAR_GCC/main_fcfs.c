...
int wt1_fcfs, wt2_fcfs, wt3_fcfs, tt1_fcfs, tt2_fcfs, tt3_fcfs;
...
static void statistics(void *pvParameters){
	printf("Waiting time task1 = %dms, turnaround time = %dms, response time = %dms\n", pdTICKS_TO_MS(wt1_fcfs), pdTICKS_TO_MS(tt1_fcfs), pdTICKS_TO_MS(wt1_fcfs));
	printf("Waiting time task2 = %dms, turnaround time = %dms, response time = %dms\n", pdTICKS_TO_MS(wt2_fcfs), pdTICKS_TO_MS(tt2_fcfs), pdTICKS_TO_MS(wt2_fcfs));
	printf("Waiting time task3 = %dms, turnaround time = %dms, response time = %dms\n", pdTICKS_TO_MS(wt3_fcfs), pdTICKS_TO_MS(tt3_fcfs), pdTICKS_TO_MS(wt3_fcfs));
	printf("Average Waiting Time = %dms, average Turnaround Time = %dms, average Response Time = %dms\n", (wt1_fcfs+wt2_fcfs+wt3_fcfs)/3, (tt1_fcfs+tt2_fcfs+tt3_fcfs)/3, (wt1_fcfs+wt2_fcfs+wt3_fcfs)/3);
	vTaskDelete(NULL);
}
/*-----------------------------------------------------------*/
